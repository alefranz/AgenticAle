...
Use the installed `agenticale-*` custom agents listed below, not generic built-in agents. Each role gets a fresh context and a
bounded task:
...
| Per-slice independent review | `agenticale-review` | Adversarial verification in a context independent from the worker. |
| Periodic integration audit / escalated slice review | `agenticale-deep-review` | Cross-slice review in a fresh context. |
| Stuck decision second opinion | `agenticale-consult` | One read-only recommendation on a decision the loop cannot settle. |
...
Implementation slices default to `agenticale-implement`. Route a slice to
`agenticale-implement-hard` only when it is genuinely reasoning-heavy:
...
3. **[strict] Review round.** Compose the review template with the task
   ...
   change range. A fresh `agenticale-review` subagent verifies independently and reports
...
at the seams between slices. Run one foreground `agenticale-deep-review`
...
Blocking findings each receive a dedicated `agenticale-fix` round; judgmental
...
before archiving it, run one foreground `agenticale-deep-review` round over
...
a question to the user: it is one foreground `agenticale-consult` round —
...
a child session: never ask the user anything (this child must not ask questions or wait for confirmation); if the evidence cannot settle a claim,
...
unattended in a child session: never ask the user anything (this child must not ask questions or wait for confirmation); if the evidence cannot
